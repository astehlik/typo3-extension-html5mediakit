...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/html5mediakit/${1}"
else
    case ${TEST_SUITE} in
        functional)
            TEST_FILE="Web/typo3conf/ext/html5mediakit/Tests/Functional"
            ;;
        unit)
            TEST_FILE="Web/typo3conf/ext/html5mediakit/Tests/Unit"
            ;;
    esac
fi
